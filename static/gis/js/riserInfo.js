...
function showRiserData(data, feature) {
    ...
    const renderRiserInfo = () => {
        ...
        loopable.forEach((_, index) => {
            if (data[`customer_id_${20 + index}`]) {
                jsx += `<tr class="text-center">
                            <td>${temp++}</td>
                            <td>${data[`customer_id_${20 + index}`]}</td>
                            <td>${data[`customer_name_${20 + index}`]}</td>
                            <td>${data[`status_${20 + index}`]}</td>
                            <td>${data[`burner_type_${20 + index}`]}</td>
                            <td>${data[`burner_no_field_${20 + index}`]}</td>
                        </tr>`
            }
        })
        return jsx
